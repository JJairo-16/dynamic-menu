...
```powershell
javac -cp "lib/dynamic-menu-1.3.jar" -d out (Get-ChildItem -Recurse -Filter *.java src).FullName
```

Execució:

```powershell
java -cp "out;lib/dynamic-menu-1.3.jar" com.example.Main
```
...
```bash
javac -cp "lib/dynamic-menu-1.3.jar" -d out $(find src -name "*.java")
```
...
```groovy
dependencies {
    implementation 'com.example:dynamic-menu:1.3'
}
```
